...
router.post(
  ...
  [
    ...
    body("password").notEmpty().trim().isLength({ min: 8, max: 20 }),
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        const { rows } = await EmployeeRepo.createWithClient(
          client,
          obj.name,
          password,
          obj.email,
        ...
        console.log(rows);
        const sal = await CurrentSalaryRepo.createWithClient(
          client,
          rows[0].id,
          obj.salary.toString(),
          req.currentUser.id
        ...
        console.log(e);
        return next(
          new BadRequestError("transaction failed during create employee")
        );
      } finally {
        client.release();
      }
